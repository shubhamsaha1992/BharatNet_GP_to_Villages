...
for thisVil in VilUniq :
    ...
    if thisVilID in valdictVilCon.keys():
        ...
        countVilLit += 1
    elif thisVilID not in dictVilToGPAll.keys():
...
nx.draw_networkx_nodes(GR_GP2Vil,pos_nodes,nodelist=node_GP_unlit, node_size=5, node_color = 'y',node_shape='o', linewidths = 0)
...
plt.legend(["Lit GPs","Unlit GPs","Connected Villages","Unconnected Villages"],prop={'size':9})



base_filename = sys.argv[1]
full_filename = os.getcwd() + '/Data_Correct_All-Vil/' +base_filename + '/'+base_filename+'.jpg'

plt.savefig(full_filename, format='jpg', dpi=1000)
#plt.show()
...
rowDist = [nameDist,numDist,countVil,countVilLit,countGP,countGPLit,conRatio]
