...
def calcSINR(dictVilToGPAll,dictVilCon,listGPCon,thisVilID,thisGPID): #Pass list of already connected Villages and this GP as signaler
	...
	for thisVilID in keysVilCon:
		...
		keysVilGP = list(dictVilToGPAll[thisVilID][1].keys())
		for thisVilGPID in keysVilGP:
			#print "		thisVilGPID ", thisVilGPID 
			if thisVilGPID == thisVilGPConID:
				thisVilSig = dictVilToGPAll[thisVilID][1][thisVilGPID]	
				#print "Sig from ", thisVilGPID, "is ", thisVilSig
			elif thisVilGPID in listGPCon:
				thisVilIntf = addIntf(thisVilIntf,dictVilToGPAll[thisVilID][1][thisVilGPID])
...
def calcNextVilList(V_n,n,dictVilToGPAll,valLinkOnListVilSINR,vallistGPCon,valdictVilCon):
...
def rewardSINR(V_n,dictVilToGPAll,valLinkOnListVilSINR,vallistGPCon,valdictVilCon):
	...
	for keyVil in keysVilAll:
		value = dictVilToGPAll[keyVil]
		maxLinkOnListVilSINR = []
		thisVilID = keyVil
		thisVil = value[0]
		thisVilLoc = thisVil[0]
		...
		if maxSINRGPID != -1:
			#print maxSINRGPID,dictGPThpt[maxSINRGPID]
			dictGPThpt = reducThptBy(maxSINRGPID,thisVilReqThpt,dictGPThpt)
			#print maxSINRGPID,dictGPThpt[maxSINRGPID]
			if(dictGPThpt[maxSINRGPID] < 0): 
				print "Error"
				wait = input("PAUSED")
				
		#print "Village", thisVilID, "connected to", maxSINRGPID, "with total SINR", maxLinkOnTotalSINR
	    #print maxLinkOnListVilSINR
	if (maxLinkOnListVilSINR != []):
	    valLinkOnListVilSINR = maxLinkOnListVilSINR
	    vallistGPCon = listGPCon
	    valdictGPThpt = dictGPThpt
	    valdictVilCon = dictVilCon
	    #print valdictGPThpt
