...
#Read population data
	...
	d = {}
	for row in reader:
		row[9] = row[9].strip().lower()
		if row[9].lower() in d.keys():

			d[row[9]]['TOT_P'] = str(int(d[row[9]]['TOT_P']) + int(row[5]))
			# print "d[row[9]]['TOT_P'] += int(row[5])",d[row[9]],d[row[9]]['TOT_P']
		else:	
	...
	return '555_first_match.csv', 'GPs_unmatched_op.csv'
